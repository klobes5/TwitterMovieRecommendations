# -*- coding: utf-8 -*-
"""
Created on Wed May 11 11:04:35 2016
...
#tweets = []
#ids = []
#for line in open('movie_tweets.txt').readlines():   
...
#print y
#print dsfa
clf = pickle.loads(open('trained_project_classifier.pkl').read())
vocab = pickle.loads(open('project_vocab.pkl').read())
print vocab
tweets = []
ids = []
for line in open('@LeoDiCaprio.txt').readlines():   
    data = json.loads(line)
    if data['text'] in ids:
        continue
    ids.append(data['text'])
...
#Load in the pickle file from the other program.
